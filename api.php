...
if ($method === 'GET' && $op === 'download_signed' && isset($_GET['codigo']) && isset($_GET['user_id'])) {
    ...
    if (file_exists($signedDbFile)) {
        ...
        $docs = is_array($signedDb) ? ($signedDb[$userId] ?? []) : [];
        foreach ($docs as $doc) {
            if (isset($doc['code']) && $doc['code'] === $codigo) {
                $found = $doc;
                break;
            }
        }
    ...
    if ($found) {
        ...
        if (isset($found['filePath']) && file_exists($found['filePath'])) {
            header('Content-Type: application/pdf');
...
if ($method === 'POST' && $op === 'sign_upload' && isset($_GET['codigo']) && isset($_GET['user_id'])) {
    ...
    // Guardar archivo localmente (Laragon / entorno local)
    $fileName = uniqid('upload_') . '.pdf';
    $targetPath = $uploadDir . '/' . $fileName;
    $localSaved = file_put_contents($targetPath, $rawData) !== false;
    ...
    if (is_string($token) && $token !== '') {
        require_once __DIR__ . '/blob_storage.php';
        $blobPathname = 'signed/' . $codigo . '_' . $userId . '_' . time() . '.pdf';
        $up = blobUploadPdf($blobPathname, $rawData);
        if ($up['ok'] && $up['url']) {
            $blobUrl = $up['url'];
        } else {
            file_put_contents($uploadDir . '/debug.log', date('c') . " blob upload fallo: " . json_encode($up) . PHP_EOL, FILE_APPEND);
        }
    }

    if (!$localSaved && $blobUrl === null) {
        http_response_code(500);
        echo json_encode(['error' => 'Error al guardar archivo']);
        exit;
    }
    ...
    if ($blobUrl) {
        $record['url'] = $blobUrl;
        $record['blobPathname'] = $blobPathname;
    }
    $signedDb[$userId][] = $record;
    @file_put_contents($signedDbFile, json_encode($signedDb, JSON_PRETTY_PRINT));
